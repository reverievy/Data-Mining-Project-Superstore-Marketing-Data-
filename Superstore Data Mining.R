...
#target variable has to be a factor with 2 levels
model=glm(superstore_data_log$Response~.,family="binomial",data = superstore_data_log)
summary(model)

#check log model's accuracy
superstore_data_log$preidcted=model$fitted.values
superstore_data_log$predictedbinary = ifelse(superstore_data_log$preidcted>0.5,1,0)
superstore_data_log$correct=ifelse(superstore_data_log$Response==superstore_data_log$predictedbinary,"Correct","Incorrect")

table1=table(superstore_data_log$Response,superstore_data_log$predictedbinary)
(table1[1,1]+table1[2,2])/nrow(superstore_data_log)
